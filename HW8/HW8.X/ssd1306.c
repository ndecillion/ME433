...
void ssd1306_printChar(unsigned char x, unsigned char y, unsigned char character){
    ...
    for (i = 0; i < 5; ++i){
        for (j = 0; j < 8; ++j){
            ...
            if (((ASCII[character - 0x20][i])>>j)&1 == 1){
...
void ssd1306_drawMessage(unsigned char x, unsigned char y, uint8_t *message){
    char worker = 0;
    while (message[worker] != 0){
        if (x >= 128){
            x = 0;
            y = y + 8;
        }
        ssd1306_printChar(x, y, message[worker]);
        worker++;
        x = x + 5;
    }
}
